...
class SimpleStorageService:
    ...
    def s3_key_path_available(self, bucket_name, s3_key) -> bool:
        ...
        try:
            ...
            # bucket.objects.filter(prefix=s3_key) this find the any object in this s3 bucket thats starts with s3_key (prefix)
            file_objects = [file_object for file_object in bucket.objects.filter(Prefix=s3_key)]
            return len(file_objects) > 0
        except Exception as e:
            raise MyException(e, sys)

    @staticmethod  # read_object  
    def read_object(object_name: str, decode: bool = True, make_readable: bool = False) -> Union[StringIO, str]:
        """
        take object return either ML model that lies on object body or data lies on object body (but in readable format)
        ...
        try:
            ...
            # decode convert b'hello' (binary) to "hello" (string) 
            # but in ML model case ( when you are reading/getting ML model from S3 bucket ) 
            # we take the model in binary form so decode == False else in data case decode True

            # now pandas reads string in object form so if the data in string (we have converted the binary to string so it would be in string form) we have to convert the string to StringIO so that pandas can read this in dataframe form 
    ...
    def get_file_object(self, filename: str, bucket_name: str) -> Union[List[object], object]:
        """
        take filename and bucketname --> check if bucket is in s3 if yes give the first matching 
        file with filename as prefix
...
############################################################################ VERY IMP ############################################
    """
        ##################### Before load lets understand the entire analog 

        Step	                        Data Type

        Original model	                Python object
        After dump	                    Binary bytes
        Stored in S3	                Binary bytes (this is what actually stored in S3 )

        get_file_object(get file object)--> returns file obj 
        After read_object	            Binary bytes
        After pickle.loads	            Python object again
    ...
    def get_df_from_object(self, object_: object) -> DataFrame:
        ...
        try:
            ...
            df = read_csv(content, na_values="na")   # this read csv is generic method to make csv into dataframe
